...
string_t	rsa(const string_t *in, const arguments_t *args) {
	...
	if (get_flag(args->flags, "-pubin")->present) {
		...
		if (get_flag(args->flags, "-breakit")->present) {
			rsa_private_key_t *recovered_private_key = break_rsa_public_key(key);
			if (recovered_private_key) {
				out = serialize_rsa_private_key(recovered_private_key);
				free(recovered_private_key);
			}
		} else {
		...
		if (get_flag(args->flags, "-pubout")->present) {
			rsa_public_key_t public_key = rsa_private_to_public_key(key);
			out = serialize_public_key(&public_key);
		} else {
