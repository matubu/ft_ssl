...
rsa_public_key_t *deserialize_rsa_private_key_as_public_key(string_t key) {

	rsa_private_key_t *private_key = deserialize_rsa_private_key(key);

	if (private_key == NULL) {
		return (NULL);
	}

	rsa_public_key_t *public_key = malloc(sizeof(rsa_public_key_t));
	*public_key = rsa_private_to_public_key(private_key);
